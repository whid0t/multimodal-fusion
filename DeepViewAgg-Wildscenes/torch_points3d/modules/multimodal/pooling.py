...
class DeepSetFeat(nn.Module, ABC):
    ...
    def forward(self, x, csr_idx):
        # DEBUG PRINT ADDED - Fixed to access correct layer
        first_linear = None
        for module in self.mlp_elt_1:
            if hasattr(module, 'in_features'):
                first_linear = module
                break
        
        if first_linear is not None:
            print(f"DEBUG DeepSetFeat: x.shape = {x.shape}, self.d_in = {self.d_in}, first_linear.in_features = {first_linear.in_features}, first_linear.weight.shape = {first_linear.weight.shape}")
        else:
            print(f"DEBUG DeepSetFeat: x.shape = {x.shape}, self.d_in = {self.d_in}, mlp_elt_1 type = {type(self.mlp_elt_1)}")
