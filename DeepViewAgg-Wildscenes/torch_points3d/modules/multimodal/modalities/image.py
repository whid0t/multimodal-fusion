...
class ResNetUp(ResNetDown, ABC):
    ...
    def forward(self, x, skip):
        ...
        # Resize skip connection if necessary to match x's shape
        if skip is not None and x.shape[2:] != skip.shape[2:]:
            x_shape = x.shape[2:]
            skip = F.interpolate(skip, size=x_shape, mode='bilinear', align_corners=False)

        # Concatenate and process through residual blocks
        if self.skip_first:
            if skip is not None:
