...
def getGroundTruth(groundTruthListFile, eval_every=1): # Changed to 1 for correct comparison with the 3D data
...
# -----------------------------
# Additional metrics (Accuracy, Precision, Recall, Dice, Trueness)
# -----------------------------
def _valid_eval_label_ids(args):
    return [
        l for l in args.evalLabels
        if not trainId2label[l].ignoreInEval
    ]

def compute_additional_metrics(confMatrix, args):
    valid_labels = _valid_eval_label_ids(args)
    if len(valid_labels) == 0:
        return {}

    # Confusion matrix layout: rows = GT, columns = Pred
    diag = np.diag(confMatrix).astype(np.float64)

    # Keep only valid label indices
    tp = diag[valid_labels]
    row_sum = confMatrix[valid_labels, :].sum(axis=1).astype(np.float64)  # GT positives
    col_sum = confMatrix[:, valid_labels].sum(axis=0).astype(np.float64)  # Pred positives
    ...
    # Overall accuracy computed on valid GT rows
    total_correct = np.sum(tp)
    total_gt = np.sum(row_sum)
    overall_acc = float(total_correct / total_gt) if total_gt > 0 else float('nan')

    def _nanmean(x):
        return float(np.nanmean(x)) if np.sum(~np.isnan(x)) > 0 else float('nan')

    metrics = {
        'overall_accuracy': overall_acc,
        'precision_per_class': {trainId2label[l].name: float(precision[i]) for i, l in enumerate(valid_labels)},
        'recall_per_class': {trainId2label[l].name: float(recall[i]) for i, l in enumerate(valid_labels)},
        'dice_per_class': {trainId2label[l].name: float(dice[i]) for i, l in enumerate(valid_labels)},
        'mPrecision': _nanmean(precision),
        'mRecall': _nanmean(recall),
        'mDice': _nanmean(dice),
    }
    return metrics
...
def evaluateImgLists(predictionImgList, groundTruthImgList, args):
    ...
    # Compute and append additional metrics
    extra = compute_additional_metrics(confMatrix, args)
    allResultsDict["additionalMetrics"] = extra
    writeJSONFile( allResultsDict, args)

    # Print summary of additional metrics
    if (not args.quiet):
        print("\nAdditional metrics (computed from confusion matrix):")
        print("  Overall accuracy: {:.4f}".format(extra.get('overall_accuracy', float('nan'))))
        print("  mPrecision (macro): {:.4f}".format(extra.get('mPrecision', float('nan'))))
        print("  mRecall (macro):    {:.4f}".format(extra.get('mRecall', float('nan'))))
        print("  mDice (macro):      {:.4f}".format(extra.get('mDice', float('nan'))))
        print("  mTrueness (macro):  {:.4f}".format(extra.get('mTrueness', float('nan'))))
...
def main():
    ...
    if len(argv) == 0:
        ...
        groundTruthImgList = getGroundTruth(args.groundTruthListFile) # Removed the original [:2] for running the full inference
