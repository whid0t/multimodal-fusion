# ------------------------------------------------------------
# Fine-tune DeepLab V3+ R-101-D8 on KITTI-360 (40 k iters)
# ------------------------------------------------------------
# mmsegmentation v0.14.1  •  torch 1.7.1  •  CUDA 11.0
# ------------------------------------------------------------
# Training was ran on the university's jupyter server
# ------------------------------------------------------------
...
model = dict(
    type='EncoderDecoder',
    pretrained='open-mmlab://resnet101_v1c',         # ignored, we load_from
    backbone=dict(
        type='ResNetV1c',
        depth=101,
        num_stages=4,
    ...
    decode_head=dict(                                # DeepLabV3+ head
        ...
        c1_channels=48,
        dropout_ratio=0.1,
        num_classes=19,                              # KITTI-360 / Cityscapes num of classes
        norm_cfg=norm_cfg,
        align_corners=False,
        loss_decode=dict(
...
data = dict(
    samples_per_gpu=2, # batch size 4 Ran into OOM with an A10 with 23GB
    workers_per_gpu=4,
    train=dict(
        type=dataset_type,
        data_root=data_root,
...
# ---- 5. FP16 (optional – comment out if you stay FP32) -----------------
...
work_dir = './work_dirs/deeplabv3plus_r101-d8_k360_40k_normal'
